// --- 0. INITS (Configuración optimizada para Android y Web) ---
...
// Crear canvas manualmente para asegurar control total en Android
const canvas = document.createElement('canvas');
canvas.id = "game-canvas";
// Aseguramos estilos para evitar el estiramiento borroso
canvas.style.width = "100%";
canvas.style.height = "100%";
canvas.style.display = "block";
canvas.style.imageRendering = "pixelated"; // Mantiene el estilo retro nítido

// Inicializar Renderer vinculado al canvas
const renderer = new THREE.WebGLRenderer({ 
    canvas: canvas,
    antialias: false,
    alpha: false,
    preserveDrawingBuffer: true // Vital para evitar pantalla negra en ciertos móviles
});

renderer.setSize(RETRO_WIDTH, RETRO_HEIGHT, false); 
renderer.setPixelRatio(window.devicePixelRatio);

// Inserción en el contenedor del DOM
